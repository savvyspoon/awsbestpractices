...
policy "ABP-v1.00" {
  ...
  policy "aws-identify" {
    ...
    query "APIGateway.2" {
      ...
      LEFT OUTER JOIN public.aws_apigatewayv2_apis  as b ON a.cq_id = b.cq_id where client_certificate_id is NULL FROM public.aws_autoscaling_launch_configurations where instance_monitoring_enabled != true
    EOF
    }
